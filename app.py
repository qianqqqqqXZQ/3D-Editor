...
function createInfiniteGrid(){const geometry=new THREE.PlaneBufferGeometry(10000,10000);const material=new THREE.ShaderMaterial({uniforms:{minorColor:{value:new THREE.Color(0x1b2940)},majorColor:{value:new THREE.Color(0x385071)}},vertexShader:'varying vec3 vWorldPosition;void main(){vec4 worldPosition=modelMatrix*vec4(position,1.0);vWorldPosition=worldPosition.xyz;gl_Position=projectionMatrix*viewMatrix*worldPosition;}',fragmentShader:'varying vec3 vWorldPosition;uniform vec3 minorColor;uniform vec3 majorColor;float gridLine(float coordinate,float spacing,float width){float scaled=coordinate/spacing;float distanceToLine=abs(fract(scaled-0.5)-0.5);float aa=fwidth(scaled);return 1.0-smoothstep(width+aa,width+aa*2.0,distanceToLine);}void main(){float minor=max(gridLine(vWorldPosition.x,1.0,.018),gridLine(vWorldPosition.y,1.0,.018));float major=max(gridLine(vWorldPosition.x,10.0,.035),gridLine(vWorldPosition.y,10.0,.035));float intensity=max(minor,major);if(intensity<.01)discard;gl_FragColor=vec4(mix(minorColor,majorColor,major),intensity*.78);}',extensions:{derivatives:true},transparent:true,depthWrite:false,polygonOffset:true,polygonOffsetFactor:1,polygonOffsetUnits:1});const grid=new THREE.Mesh(geometry,material);grid.userData.infiniteGrid=true;return grid}
