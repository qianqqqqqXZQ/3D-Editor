...
- [x] 增加公开的 PLY/PT 文件加载函数及规范化字段映射
- [x] 增加 `.pt` 帧保存函数，并兼容 gsplat 两种 checkpoint 结构
- [x] 增加 4DGS 目录加载、帧索引、SH degree 对齐逻辑
- [x] 增加欧拉角/矩阵/四元数与关键帧插值工具
- [x] 增加从原始数据重算的单帧/全帧变换逻辑并接入现有 API
- [x] 运行编译、单元级回归和 Flask API 验证
- [x] 完成修改后的代码审查并更新本文件
